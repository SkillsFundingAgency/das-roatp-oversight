...
public class SearchTermValidatorTests
{
    ...
    {
        var response = _validator.Validate(string.Empty);

        Assert.IsFalse(response.IsValid);
        Assert.AreEqual($"Enter an organisation name or UKPRN", response.Errors[0].ErrorMessage);
        Assert.AreEqual("SearchTerm", response.Errors[0].PropertyName);
    }
    ...
    public void When_searchTerm_is_less_than_minimum_length_then_an_error_is_returned()
    {
        ...
        Assert.IsFalse(response.IsValid);
        Assert.AreEqual($"Enter a UKPRN or an organisation name using {MinimumLength} or more characters", response.Errors[0].ErrorMessage);
        Assert.AreEqual("SearchTerm", response.Errors[0].PropertyName);
    }
