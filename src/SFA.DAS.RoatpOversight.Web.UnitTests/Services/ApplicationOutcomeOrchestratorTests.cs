...
public class ApplicationOutcomeOrchestratorTests
{
    ...
    private Mock<IRoatpOversightOuterApiClient> _roatpOversightApiClient;
    ...
    public void Before_each_test()
    {
        ...
        _roatpOversightApiClient = new Mock<IRoatpOversightOuterApiClient>();
    ...
    [TestCase(ProviderTypeConstants.Employer, 0)]
    ...
    public async Task Application_status_and_register_updated_for_a_successful_already_active_or_fitness_for_funding_oversight_review(OversightReviewStatus status)
    {
        ...
        {
            Ukprn = _registrationDetails.UKPRN,
        ...
        _applicationApiClient.Verify(x => x.RecordOutcome(It.Is<RecordOversightOutcomeCommand>(y => y.ApplicationId == _applicationId)), Times.Once);
        _roatpRegisterApiClient.Verify(x => x.UpdateOrganisation(int.Parse(_registrationDetails.UKPRN), It.IsAny<UpdateOrganisationRequest>()), Times.Once);
